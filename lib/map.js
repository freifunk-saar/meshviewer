  ...
  function (ClientLayer, LabelsLayer, d3, L, moment, LocationMarker, rbush) {
    ...
    return function (config, linkScale, sidebar, router, buttons) {
      ...
      if (localStorageTest()) {
        ...
        if (d && d.name in allLayers) {
            map.removeLayer(allLayers[currentLayer])
            currentLayer = d.name
            map.addLayer(allLayers[currentLayer])
        }
      }
      ...
      map.on("baselayerchange", function(e) {
        currentLayer = e.name
        if (localStorageTest())
          localStorage.setItem("map/selectedLayer", JSON.stringify({name: currentLayer}))
      })
