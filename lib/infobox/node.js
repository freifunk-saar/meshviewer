define(["moment", "numeral", "tablesort", "tablesort.numeric"],
  ...
  function showGateway(d, router) {
   if (!d.selectedGateway)
     return undefined

   return function (el) {
      var a = document.createElement("a")
      a.textContent = d.selectedGateway.nodeinfo.hostname
      a.classList.add("hostname")
      a.href = "#"
      a.onclick = router.node(d.selectedGateway)
      el.appendChild(a)
    }
  }
  ...
  return function(config, el, router, d) {
    ...
    attributeEntry(attributes, "Gewählter Gateway", showGateway(d, router))
